...
选型除了看额度数字，还得看产品线是在扩张还是收缩。写这份文档的时候，
两条原本的首选路都堵了：DeepL 的免费 API 开通要境外发行的信用卡，国内的 Visa
双币卡外币卡都不认；腾讯云机器翻译已公告退市，2027-10-01 起全部接口不可用。
新项目别再从这两家起步。

| 你的需求 | 用什么 |
|---|---|
| 只想快点用起来 | `mymemory`：免注册，transgo 一开始就在用 |
| 日常够用、免费额度大 | `volcano`：每月前 200 万字符免费，国内直连。超出 49 元/百万字符且**会自动扣费**，记得设额度预警 |
| 中文场景多 | `baidu`（高级版）或 `aliyun` |
| 想要可控的术语表 / 文风 | `llm`：改系统提示词就能固定术语、语气 |
| 完全离线内网 | `llm` 指向本地 Ollama |

推荐 `volcano` + `mymemory` 搭着用。`mymemory` 免注册，排在优先级末尾垫底做兜底；
`volcano` 配好后自动顶上，日常场景全覆盖。
...
**别再新接这家。** 2026-09-03 公告《QPS 调整与免费资源包下线》：
...
建议去 <https://console.volcengine.com/cost/expense> 设置额度预警：充 1 元，把「可用额度
预警」设成 1 元，扣款时就会通知你。
