...
`languages()` 返回 `None` 表示不做白名单限制（LLM、MyMemory、Google、阿里云、火山、有道）。
返回 `Some(&[...])` 的是语种范围确实有约束的：DeepL 23 个、Azure 29 个、腾讯 18 个、
百度 22 个（`baidu` 与 `baidu-llm` 共用这份白名单）。
...
规则三条：中英互转；拉丁字母系按英文方向互转；其余非拉丁文字翻向 `default.lang`。

判定在 `detect::is_chinese()` 与 `detect::auto_target()`。设计要点：

- 看整段的字符构成比例（汉字占字母类字符的 1/3 以上），不看首字。否则「iPhone手机很好用」
  这类以英文开头的中文会被误判
- 数字、标点、空白不计入统计。它们在任何语言里长得都一样，掺进来只会稀释比例、
  把判定往阈值上拉，让结果不稳定
- 拉丁字母系不做语种区分：字符上分不开英法德西，统一按英文方向互转，宁可少而准
- 假名/谚文等非拉丁文字走第三方方向，由 `default.lang`（`zh` / `en`，默认 `en`）指定

`detect::detect()` 是另一回事：它只用于**展示**（引擎不回传检测到的源语种时，
给 `-v` / `--json` 的 `from` 字段补齐），不影响译文方向。

### 已知局限

中文句子里夹一整个片假名词（「我很喜欢カタカナ」）会被判成日文。
这两种情况在字符统计上完全一样（汉字与假名各半），区分不了，
判错方向时用 `-f` / `-t` 手动覆盖。同样，法/德/西与英文在字符上分不开，
都按英文方向处理——不做更多的语种猜测。
...
- `openai.rs`：剥掉模型的加戏——外层引号（保留文本内部的）和回显的分隔线
