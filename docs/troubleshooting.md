...
| 错误码 | 含义 |
|---|---|
| `54001` | 签名错误，99% 是 APP ID 或密钥抄错了 |
| `52003` | 未授权用户，APP ID 不对 |
| `54003` / `54005` | 访问频率受限（标准版 QPS 只有 1） |
| `54004` | 余额不足，**建议去控制台开启免费额度用量提醒** |
...
`auto` 的规则是「中文译英文、其余译中文」，靠字符比例判定。边界情况会翻：

| 输入 | 判定 | 说明 |
|---|---|---|
| `iPhone手机很好用` | 判为中文，正确 | 英文开头不影响，看整段比例 |
| `那个 meeting 的议程定了` | 判为中文，正确 | 夹几个英文词不影响 |
| `The 会议 will be held tomorrow` | 判为英文，正确 | 汉字占比太低 |
| `我很喜欢カタカナ` | 判为日文，误判 | **已知局限**：夹了片假名词的中文和日文，统计上区分不了 |
